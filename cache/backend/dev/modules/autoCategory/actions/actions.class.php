...
abstract class autoCategoryActions extends sfActions
{
  ...
  public function executeNew(sfWebRequest $request)
  {
    ...
    $this->bookit_category = $this->form->getObject();
  }
  ...
  {
    $this->bookit_category = $this->getRoute()->getObject();
    $this->form = $this->configuration->getForm($this->bookit_category);
  }
  ...
  protected function processForm(sfWebRequest $request, sfForm $form)
  {
    ...
    if ($form->isValid())
    {
      ...
      $this->dispatcher->notify(new sfEvent($this, 'admin.save_object', array('object' => $bookit_category)));

      if ($request->hasParameter('_save_and_add'))
      {
        $this->getUser()->setFlash('notice', $notice.' You can add another one below.');

        $this->redirect('@bookit_category_new');
      }
      else
      {
        $this->getUser()->setFlash('notice', $notice);

        $this->redirect(array('sf_route' => 'bookit_category_edit', 'sf_subject' => $bookit_category));
      }
    }
    else
    {
  ...
  {
    $pager = $this->configuration->getPager('BookitCategory');
  ...
  {
    return Doctrine_Core::getTable('BookitCategory')->hasColumn($column);
  }
}
